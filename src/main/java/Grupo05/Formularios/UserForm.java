...
import Grupo05.Persistencia.UserDAO; // Importa la interfaz o clase UserDAO, que define las operaciones de acceso a datos para la entidad User.
import Grupo05.Utils.CBOption; // Importa la clase CBOption, probablemente una clase utilitaria para manejar opciones de un ComboBox (por ejemplo, para asociar un valor con un texto).
import Grupo05.Utils.CUD; // Importa el enum CUD (Create, Update, Delete),  para indicar el tipo de operación que se está realizando (Crear, Actualizar, Eliminar).
...
public class UserForm extends JDialog {
    ...
    public UserForm(MainForm mainForm, CUD cud, User user) {
    ...
    private void initCBStatus() {
        ...
        DefaultComboBoxModel<CBOption> model = (DefaultComboBoxModel<CBOption>) cmbEstado.getModel();
    ...
    private void setValuesControls(User user) {
        ...
        if (this.cud == CUD.CREATE) {
            ...
            this.en.setFechaCreado(now);
        } else {
            // Para actualización o eliminación, mostrar la fecha de contrato existente
            txtFecha.setText(user.getFechaCreado() != null ?
                    user.getFechaCreado().format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss")) : "");
        }

        // Seleccionar el estatus en el ComboBox 'cbStatus'.
        cmbEstado.setSelectedItem(new CBOption(null, user.getStatus()));
        ...
        if (this.cud == CUD.CREATE) {
            ...
            cmbEstado.setSelectedItem(new CBOption(null, 1));
        }
    ...
    private boolean getValuesControls() {
        ...
        CBOption selectedOption = (CBOption) cmbEstado.getSelectedItem();
        ...
        this.en.setFechaCreado(fechaContrato);
